...
class VMC_local(AbstractVariationalDriver):
    ...
    def _forward_and_backward(self): ## This is where the model is being evaluated at each step
        """
        ...
        self._loss_stats, self._loss_grad = self.state.expect_and_grad(self._ham) # the model is here
        ## self contains: n_iter (number of iterations) and _step_count (current step)
        ## _prog = (_step_count/n_iter)
